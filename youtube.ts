...
function findInitialContinuation(
  ...
  const sortMenus = [...searchDict(data, "sortFilterSubMenuRenderer")] as SortFilterSubMenuRenderer[]
  ...
  const continuationEndpoints = [...searchDict(data, "continuationEndpoint")] as ContinuationEndpoint[]
...
function parseCommentsFromMutations(
  mutations: Mutation[],
  minLikes: number
): Comment[] {
...
async function fetchComments(
  ...
  while (continuations.length > 0) {
    ...
    const response = await fetch(
      `${YOUTUBE_URL}/youtubei/v1/next?key=${apiKey}`,
      {
        method: "POST",
        headers: {
      ...
      for (const item of items) {
        ...
        if (COMMENT_SECTION_IDS.includes(targetId)) {
          ...
          if ("continuationItemRenderer" in item) {
            const buttonRenderers = [...searchDict(item, "buttonRenderer")] as Array<{ command?: ContinuationEndpoint }>
